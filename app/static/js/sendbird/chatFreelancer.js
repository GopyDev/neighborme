...
function getClientaccount(options) {
   ...
   sendbird.getUserList({
    ...
    "successFunc" : function(data) {
      ...
      $.each(users, function(index, user) {
        if (clientName == user['nickname']){
          clientId=user["guest_id"];
          startMessaging();
        }
      });
    },
...
function addMessagingChannel(isGroup, channelMemberList, targetChannel) {
  $('#messagediv').attr('disabled', false);
  ...
  sendbird.joinMessagingChannel(
    channelUrl,
    {
      "successFunc" : function(data) {
        ...
        sendbird.connect({
          "successFunc": function(data) {
            //alert("joined channel connect 595lines.");
            $('.chat-input').attr('disabled', false);
            loadMoreChatMessage(scrollPositionBottom);
            addMessagingChannel(isGroup, channelMemberList, currChannelInfo);
          },
    ...
    "successFunc": function(data) {
      ...
      $.each(data['channels'], function(index, channel) {
        ...
        $.each(channel["members"], function(index, member) {
          if (!isCurrentUser(member['guest_id'])) {
            if (clientName == member['name']){

              clientMsgChannel = channel["channel"];
            }
        ...
        if (clientMsgChannel){
          ...
          joinMessagingChannel(targetChannel["channel_url"]);
      ...
      if (!clientMsgChannel){
        ...
        while (!clientId && userListNext != 0){
          getClientaccount({"token": userListToken, "page": userListNext, "limit": 30});
        }
      }
    },
    "errorFunc": function(status, error) {
...
var scrollPositionBottom = function() {
  var scrollHeight = $('#messagediv')[0].scrollHeight;
  $('#messagediv')[0].scrollTop = scrollHeight;
...
function setChatMessage(obj) {
  $('#messagediv').append(messageList(obj));
...
function loadMoreChatMessage(func) {
  sendbird.getMessageLoadMore({
    ...
    "successFunc": function(data) {
      ...
      $('#messagediv').prepend(msgList);
      $('#messagediv')[0].scrollTop = (moreMessage.length * MESSAGE_TEXT_HEIGHT);
...
function messageList(obj) {
  ...
  if (isCurrentUser(obj['user']['guest_id'])) {
    msgList += '' +
      ...
      '<div class="col-xs-8">' + 
      '<label class="pull-right chat-background-gray">' +
      convertLinkMessage(obj['message']) +
      '</label>' +
      '</div>' +
      '<div style="float:left">' + 
      '<img class="browse-user-image chat-user-image" src="'+ obj['user']['image'] + '">'+
      '</img>' +
      '</div>' +
      '</div>';
  } else {
    msgList += '' +
      ...
      '</div>' +
      '<div class="col-xs-8">' + 
      '<label class="chat-background-pink">' +
      convertLinkMessage(obj['message']) +
      '</label>' +
      '</div>' +
      '</div>'
