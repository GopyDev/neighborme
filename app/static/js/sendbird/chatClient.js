...
function addMessagingChannel(isGroup, channelMemberList, targetChannel) {
  if ($('#offer_status').val()){
    $('#helpuserinfo').attr('disabled', false);
  }
  $('#helpuserinfo').html('');
      ...
      "successFunc" : function(data) {
        $.each(data['members'], function(index, member) {
          if (member['nickname'] != nickname){
            ...
            $('#helpuserinfo').append(
              '<div class="text-center user-image-padding" onclick="joinMessagingChannel(\'' + targetChannel["channel_url"] + '\')"' +
              '     data-channel-url="' + targetChannel["channel_url"] + '"' +
              '>' +
              '<img class="browse-user-image" src="' + 
              member['picture'] + '"' +
              'height="80" width="80"' +
              '/>' +
              '<h4>' +
...
$('#replybtn').on("click", function () {
  var chatcontent=$('.chat-input-text__field').val();

  if (chatcontent) {
    sendbird.message(chatcontent);
    scrollPositionBottom();
    $('.chat-input-text__field').val("");
  }
}); 
